...
export class Watch implements Consumer {
  ...
  notify(): void {
    if (!this.dirty) {
      console.log(`%cwatch: schedule watch ${this.id}.`, 'color: red;');
      this.schedule(this);
    }
    this.dirty = true;
  }
   ...
   */
  run(): void {
    console.log(`%cwatch: run ${this.id}.`, 'color: red;');
    ...
    console.log(`%cwatch: trackingVersion -> ${this.trackingVersion}.`, 'color: red;');
    try {
      this.watch();
    } finally {
