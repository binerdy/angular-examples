...
export function setActiveConsumer(consumer: Consumer|null): Consumer|null {
  ...
  console.log(`%cgraph: set active consumer -> ${consumer?.id}.`, 'color: violet;');
...
export function producerNotifyConsumers(producer: Producer): void {
  for (const [consumerId, edge] of producer.consumers) {
    ...
    console.log(`%cgraph: notify consumer ${consumer.id}`, 'color: violet;');
    consumer.notify();
  }
}
...
export function producerAccessed(producer: Producer): void {
  ...
  console.log(`%cgraph: producerAccessed: ${producer.id}.`, 'color: violet;');

  // Either create or update the dependency `Edge` in both directions.
  let edge = activeConsumer.producers.get(producer.id);
  if (edge === undefined) {
    console.log(`%cgraph: create edge.`, 'color: violet;');
    console.log(`%cgraph: consumer(${activeConsumer.id}) -> producer(${producer.id}).`, 'color: violet;');
    console.log(`%cgraph: seenValueVersion -> ${producer.valueVersion}.`, 'color: violet;');
    console.log(`%cgraph: atTrackingVersion -> ${activeConsumer.trackingVersion}.`, 'color: violet;');
    edge = {
    ...
    console.log(`%cgraph: update edge.`, 'color: violet;');
    console.log(`%cgraph: seenValueVersion -> ${producer.valueVersion}.`, 'color: violet;');
    console.log(`%cgraph: atTrackingVersion -> ${activeConsumer.trackingVersion}.`, 'color: violet;');
