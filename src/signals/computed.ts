...
export function computed<T>(
  ...
  console.log(`%ccomputed: new ComputedImpl ${node.id}.`, 'color: green;');
...
class ComputedImpl<T> implements Producer, Consumer {
  ...
  checkForChangedValue(): void {
    console.log(`%ccomputed: ${this.id} checkForChangedValue.`, 'color: green;');
    if (!this.stale) {
    ...
    console.log(`%ccomputed: ${this.id} is stale.`, 'color: green;');
    this.recomputeValue();
  }

  private recomputeValue(): void {
    console.log(`%ccomputed: ${this.id} recomputeValue.`, 'color: green;');
    ...
    console.log(`%ccomputed: ${this.id} trackingVersion -> ${this.trackingVersion}.`, 'color: green;');

    const prevConsumer = setActiveConsumer(this);
    let newValue: T;
    try {
      newValue = this.computation();
      console.log(`%ccomputed: ${this.id} newValue -> ${newValue}.`, 'color: green;');
    } catch (err) {
      newValue = ERRORED;
      this.error = err;
    } finally {
    ...
    this.value = newValue;
    this.valueVersion++;
    console.log(`%ccomputed: ${this.id} valueVersion -> ${this.valueVersion}.`, 'color: green;');
  }
  ...
  notify(): void {
    ...
    console.log(`%ccomputed: ${this.id} set stale.`, 'color: green;');
    this.stale = true;
  ...
  signal(): T {
    console.log(`%ccomputed: ${this.id} called.`, 'color: green;');
