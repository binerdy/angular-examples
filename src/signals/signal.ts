...
class SettableSignalImpl<T> implements Producer {
   ...
   */
  set(newValue: T): void {
    console.log(`%csignal: set value -> ${newValue}.`, 'color: blue;');
    if (!this.equal(this.value, newValue)) {
      this.value = newValue;
      this.valueVersion++;
      console.log(`%csignal: ${this.id} valueVersion -> ${this.valueVersion}.`, 'color: blue;');
  ...
  signal(): T {
    console.log(`%csignal: called.`, 'color: blue;');
...
export function signal<T>(
  ...
  console.log(`%csignal: new SettableSignalImpl ${signalNode.id}.`, 'color: blue;');
